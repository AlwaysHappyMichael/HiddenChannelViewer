...
module.exports = (() => {
	const config = {
		info: {
			...
			version: "2023.2.21",
			description: "A plugin that allow you to view all hidden channels' features except messages.",
	...
	? class {
		load() { BdApi.showConfirmationModal("ZLib Missing", `The library plugin (ZeresPluginLibrary) needed for ${config.info.name} is missing. Please click "Download Now" to install it.`, { confirmText: "Download Now", cancelText: "Cancel", onConfirm: () => this.downloadZLib(), }); }
		async downloadZLib() {
			...
			try {
				await writeFile(join(BdApi.Plugins.folder, "0PluginLibrary.plugin.js"), ZLibContent, (err) => { if (err) return this.errorDownloadZLib() });
			} catch (err) {
		...
		errorDownloadZLib() {
			...
			BdApi.showConfirmationModal(
				...
				{ confirmText: "Download", cancelText: "Cancel", onConfirm: () => { shell.openExternal("https://rauenzi.github.io/BDPluginLibrary/release/0PluginLibrary.plugin.js"); }, }
			); 
		}
		start() {}
		top() {}
	...
	: (([Plugin, Library]) => {
		...
		const Route = WebpackModules.getModule((m) => ["impressionName", "impressionProperties", "disableTrack"].every((s) => m?.Z?.toString().includes(s)));
		const ChannelItem = WebpackModules.getModule((m) => ["canHaveDot", "unreadRelevant", "UNREAD_HIGHLIGHT"].every((s) => m?.Z?.toString().includes(s)));
		const ChannelUtil = WebpackModules.getModule((m) => ["locked", "hasActiveThreads"].every((s) => m?.KS?.toString().includes(s)));
		const { rolePill, rolePillBorder } = WebpackModules.getByProps("rolePill", "rolePillBorder");
		...
		const { U: RolePill } = WebpackModules.getModule((m) => m?.U?.render?.toString().includes("roleStyle"));
		const {Z: UserMentions} = WebpackModules.getModule(m => m?.Z?.react?.toString().includes("inlinePreview"));
		const CategoryUtil = WebpackModules.getModule((m) => m?.c4?.toString().includes("CATEGORY_COLLAPSE"));
		const CategoryStore = WebpackModules.getByProps("isCollapsed", "getCollapsedCategories");
		const ChannelUtils = {
			filter: ["channel", "guild"],
			get Module() { return WebpackModules.getModule((m) => this.filter.every((s) => m?.v0?.toString().includes(s))); },
			get ChannelTopic() { return this.Module.v0; },
		};
		const ChannelTypes = [
			"GUILD_TEXT",
			"GUILD_VOICE",
			...
			render() {
				return React.createElement(
					...
					React.createElement(
						...
						this.props.icon 
						&&
						React.createElement("img", { src: this.props.icon, width: 32, height: 32, style: { borderRadius: "360px", }, }),
						React.createElement("div", { style: { display: "inline", fontSize: "22px", position: "relative", bottom: "7.5px", left: "2.5px", }, }, this.props.children)
					)
				);
			}
		}
		...
		return class ShowHiddenChannels extends Plugin {
			...
			Patch() {
				...
				Patcher.after(ChannelPermissionStore, "can", (_, args, res) => {
					...
					if (args[0] == DiscordConstants.Plq.VIEW_CHANNEL) return (!this.settings["blacklistedGuilds"][args[1].guild_id] && this.settings["channels"][DiscordConstants.d4z[args[1].type]]);
				...
				if (!this.settings.disableIcons) {
					Patcher.after(ChannelItem, "Z", (_, args, res) => {
						...
						if (instance.channel?.isHidden()) {
							...
							const children = Utilities.findInReactTree(res, (m) => m?.props?.onClick?.toString().includes("stopPropagation"));
							if (children.props?.children) {
								children.props.children = [
								  React.createElement(
									...
									React.createElement(
										"div",
										{ ...props, className: `${iconItem}`, style: { display: "block", }, },
										this.settings["hiddenChannelIcon"] == "lock" 
										&&
										React.createElement(
											"svg",
											{ class: actionIcon, viewBox: "0 0 24 24", },
											React.createElement("path", { fill: "currentColor", d: "M17 11V7C17 4.243 14.756 2 12 2C9.242 2 7 4.243 7 7V11C5.897 11 5 11.896 5 13V20C5 21.103 5.897 22 7 22H17C18.103 22 19 21.103 19 20V13C19 11.896 18.103 11 17 11ZM12 18C11.172 18 10.5 17.328 10.5 16.5C10.5 15.672 11.172 15 12 15C12.828 15 13.5 15.672 13.5 16.5C13.5 17.328 12.828 18 12 18ZM15 11H9V7C9 5.346 10.346 4 12 4C13.654 4 15 5.346 15 7V11Z", }),
										),
										...
										React.createElement(
											...
											React.createElement("polygon", { fill: "currentColor", points: "22.6,2.7 22.6,2.8 19.3,6.1 16,9.3 16,9.4 15,10.4 15,10.4 10.3,15 2.8,22.5 1.4,21.1 21.2,1.3 ", })
										),
				...
				Patcher.after(CategoryStore, "getCollapsedCategories", (_, args, res) => { return { ...res, ...this.collapsed }; });
				Patcher.after(CategoryStore, "isCollapsed", (_, args, res) => { 
					if (!args[0]?.endsWith("hidden")) { return res; }
					if (!this.settings["alwaysCollapse"]) { return this.collapsed[args[0]]; }
				...
				Patcher.after(ChannelListStore, "getGuild", (_, args, res) => {
					if (this.settings.debugMode) Logger.info(res)
					...
					switch (this.settings["sort"]) {
						...
						case "extra": {
							...
							const hiddenChannels = this.getHiddenChannelRecord([res.guildChannels.favoritesCategory, res.guildChannels.recentsCategory, res.guildChannels.noParentCategory, ...Object.values(res.guildChannels.categories).filter((m) => m.id !== hiddenId), ], args[0]);
							const GuildCategories = GuildChannelsStore.getChannels(args[0])[DiscordConstants.d4z.GUILD_CATEGORY];
							const hiddenCategoryChannel = new Channel({ guild_id: args[0], id: hiddenId, name: "Hidden Channels", type: DiscordConstants.d4z.GUILD_CATEGORY, });
							Object.defineProperty(hiddenCategoryChannel, 'position', { value: (GuildCategories[GuildCategories.length - 1] || { comparator: 0,}).comparator - 1, writable: true });
							const HiddenCategory = new Category(res.guildChannels, hiddenCategoryChannel, hiddenChannels.channels, res.guildChannels.initializationData);
							...
							if (Object.values(HiddenCategory.channels).length) { res.guildChannels.categories[hiddenId] = HiddenCategory; }
							break;
						}
					}
					if (this.settings["shouldShowEmptyCategory"]) {this.patchEmptyCategoryFunction([...Object.values(res.guildChannels.categories).filter((m) => !m.id.includes("hidden")), ]); }
				});
			  ...
			  return React.memo((props) => {
				...
				return React.createElement(
				  ...
				  React.createElement(
					...
					React.createElement("img", { style: { WebkitUserDrag: "none", maxHeight: 128, }, src: "https://raw.githubusercontent.com/Tharki-God/files-random-host/main/unknown copy.png", }),
					React.createElement(TextElement, { color: TextElement.Colors.HEADER_PRIMARY, size: TextElement.Sizes.SIZE_32, style: { marginTop: 20, fontWeight: "bold" }, }, "This is a hidden channel."),
					React.createElement(TextElement, { color:  TextElement.Colors.HEADER_SECONDARY, size: TextElement.Sizes.SIZE_16, style: { marginTop: 16 }, }, "You cannot see the contents of this channel. ", props.channel.topic && props.channel.type != 15 && "However, you may see its topic."),
					//* Topic
					props.channel.topic && props.channel.type != 15 && props.guild && ChannelUtils?.ChannelTopic(props.channel, props.guild),
					//* Last message
					props.channel.lastMessageId && React.createElement(TextElement, { color: TextElement.Colors.INTERACTIVE_NORMAL, size: TextElement.Sizes.SIZE_14, }, "Last message sent: ", this.getDateFromSnowflake(props.channel.lastMessageId)),
					//* Slowmode
					props.channel.rateLimitPerUser > 0 && React.createElement(TextElement, { color: TextElement.Colors.INTERACTIVE_NORMAL, size: TextElement.Sizes.SIZE_14, style: { marginTop: 10, }, }, "Slowmode: ", this.convertToHMS(props.channel.rateLimitPerUser)),
					//* NSFW
					props.channel.nsfw && React.createElement(TextElement, { color: TextElement.Colors.INTERACTIVE_NORMAL, size: TextElement.Sizes.SIZE_14, style: { marginTop: 10, }, }, "Age-Restricted Channel (NSFW)"),
					//* Permissions
					this.settings["showPerms"] && props.channel.permissionOverwrites &&
					React.createElement("div", { style: { marginTop: 20, backgroundColor: "var(--background-secondary-alt)", padding: 10, borderRadius: 5, color: "var(--text-normal)", }, }, 
					//* Users
					React.createElement(TextElement, { color: TextElement.Colors.INTERACTIVE_NORMAL, size: TextElement.Sizes.SIZE_14, }, "Users that can see this channel: ", ),
					React.createElement("div", { style: { color: "var(--interactive-normal)",  marginTop: 5, marginBottom: 5, }, }, ...(() => { const allUsers = Object.values(props.channel.permissionOverwrites).filter(user => (user !== undefined && user?.type == 1) && (user.allow && BigInt(user.allow)) && GuildMemberStore.isMember( props.guild.id, user.id)); if (!allUsers?.length) return ["None"]; return allUsers.map(m => UserMentions.react({ userId: m.id, channelId: props.channel.id }, NOOP, { noStyleAndInteraction: false, })); })()),
					//* Roles
					React.createElement(TextElement, { color: TextElement.Colors.INTERACTIVE_NORMAL, size: TextElement.Sizes.SIZE_14, style: { marginTop: 10, marginBottom: 5, }, }, "Roles that can see this channel: ",),
					React.createElement("div", { style: { color: "var(--interactive-normal)", }, }, ...(() => {
						...
						if (this.settings["showAdmin"]){ Object.values(props.guild.roles).forEach(role => { if(((role.permissions & BigInt(8)) == BigInt(8) && !allRoles.find(r => r.id == role.id)) && (this.settings["showAdmin"] == "include" || (this.settings["showAdmin"] == "exclude" && !role.tags?.bot_id))) { allRoles.push(role); }}); }
						if (!allRoles?.length) return ["None"];                      
						return allRoles.map(m => RolePill.render({ canRemove: false,className: `${rolePill} shc-rolePill`, /*${rolePillBorder}*/ disableBorderColor: true, guildId: props.guild.id, onRemove: NOOP, role: props.guild.roles[m.id] }, NOOP));
					})()), ),
			...
			getSettingsPanel() {
			  return SettingPanel.build(
				...
				new Switch(
					...
					(i) => { this.settings["showPerms"] = i; }
				),
				new RadioGroup(
					"Show Administrator Roles",
					...
					[
					  { name: "Do not Show Administrator Roles", value: false, },
				...
				new SettingGroup( "Guilds Blacklist", { collapsible: true, shown: false }).append(
